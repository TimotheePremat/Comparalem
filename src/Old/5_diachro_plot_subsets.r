...
combined_data <- tk_choose.files(default = file.path(default_folder),
...
grouped <- combined_data %>% group_by(cat)
summarise(grouped, mean = mean(type), sd = sd(type))

p_by_cat <- ggplot(combined_data, aes(x = cat, fill = type_full)) +
