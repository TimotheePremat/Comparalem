...
#Save plots
	...
	ggsave(glm2_plot_accuracy,
			path = "Graphs",
			filename = paste(file_name, "_GLM_by_occ_predict_obs", ".png", sep=""),
			width=9,
			height = 9,
			units = "in")
	ggsave(glm2_plot_estimate,
			path = "Graphs",
			filename = paste(file_name, "_GLM_by_occ_estimate", ".png", sep=""),
			width=9,
			height = 9,
			units = "in")
...
#Save tables
	...
	write.table(glm2_sum_output,
				file=paste("Tables/",file_name, "_GLM_by_occurrence.txt", sep=""),
				quote=FALSE)
	#Save general rate by text data
