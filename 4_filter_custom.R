...
###### ANTICIPATE REGEX SUPPRESSION NOT TO BLOCK ON NAME CHECKING LOOPING BELOW
	...
	#check correct variable and value
		...
		#Check for consistency of negation and Define vector_status (needed for branching below: exclusion or inclusion)
		...
		#Define vector_regex and vector_multiple_values
		...
		if (vector_regex == "yes") {									# If regex activated
			check_value_exists_regex(Data_all,
									filtering_var,
									filtering_value)
		} else if (vector_regex == "no") {								# If regex non activated
	...
	#filter based on the variable-value couple
		...
		#Branch depending on positive (filter in) or negative (filter out) value
		if (vector_regex == "yes") {											#If regex match required
			if (vector_status == "neg") {										#And if filtering by exclusion
		...
		} else if (vector_status == "neg") {									#If not regex and filtering by exclusion
